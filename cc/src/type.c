...
symn lookup(ccContext cc, symn sym, astn ref, astn args, int raise) {
	...
	for (; sym; sym = sym->next) {
		...
		if (args != NULL) {
			...
			// make list of args chained by next
			while (tree != NULL) {
				astn arg = tree;
			...
			while (argval && param) {
				...
				if (argval->kind == TOKEN_var && argval->ref.link == cc->null_ref) {
					hascast += 1;
				}
			...
			if (sym->params != NULL && (argval || param)) {
		...
		debug("%t%s is probably %T", ref, args ? "()" : "", sym);
	}
...
ccKind canAssign(ccContext cc, symn var, astn val, int strict) {
	...
	if (lnk == cc->null_ref) {
		if (typ != NULL) {
			...
			if ((typ->kind & MASK_cast) == CAST_ref) {
				return CAST_ref;
			}
	...
	// Assign array
	if ((typ->kind & MASK_cast) == CAST_arr) {
		struct astNode atag;
		symn vty = val->type;
				...
				case CAST_u64:
					switch (castKind(lht)) {
						...
						case CAST_u64:
							...
							if ((lht->kind & MASK_cast) == CAST_bit && lht->size == rht->size) {
								result = rht;
							}
							else {
