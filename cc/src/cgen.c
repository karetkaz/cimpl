...
static size_t emitOperator(rtContext rt, vmOpcode opc, ccKind cast) {
	if (opc == opc_ceq) switch (cast) {
		...
		if (!emitOperator(rt, opc_ceq, cast)) {
			trace(ERR_INTERNAL_ERROR);
			return 0;
		}
		opc = opc_not;
		...
		if (!emitOperator(rt, opc_clt, cast)) {
			trace(ERR_INTERNAL_ERROR);
			return 0;
		}
		opc = opc_not;
		...
		if (!emitOperator(rt, opc_cgt, cast)) {
			trace(ERR_INTERNAL_ERROR);
			return 0;
		}
		opc = opc_not;
...
//		debug("%?s:%?u: initialize variable: %T := %t", variable->file, variable->line, variable, variable->init);
	...
	if (variable->offs == 0) {
		if (variable->init == NULL) {
			if (!emitInt(rt, opc_spc, variable->size)) {
				return TYPE_any;
			}
		}
...
static inline ccKind genCall(rtContext rt, astn ast) {
	...
	ccKind result = ast->type->kind & MASK_cast;
	...
	if (function->params != NULL && args != NULL) {
		...
		astn next = NULL, arg = args;
		while (arg->kind == OPER_com) {
			arg->op.rhso->next = next;
			next = arg->op.rhso;
			arg = arg->op.lhso;
		}
		arg->next = next;
...
static inline ccKind genIndex(rtContext rt, astn ast, ccKind get) {
	...
	return ast->type->kind & MASK_cast;
}
static inline ccKind genMember(rtContext rt, astn ast, ccKind get) {
	...
	if ((member->kind & MASK_cast) == CAST_ref) {
		if (!emitInt(rt, opc_ldi, vm_size)) {
			traceAst(ast);
			return TYPE_any;
		}
...
static ccKind genAst(rtContext rt, astn ast, ccKind get) {
	...
	switch (ast->kind) {
		...
		case OPER_cmt: {	// '~'
			...
			switch (ast->kind) {
				...
				case OPER_mns:
					...
					break;
				case OPER_not:
					dieif(got != CAST_bit, ERR_INTERNAL_ERROR": got: %K", got);
					opc = opc_not;
					break;
				case OPER_cmt:
					opc = opc_cmt;
		...
		case OPER_mod: {	// '%'
			...
			if (!(got = genAst(rt, ast->op.lhso, TYPE_any))) {
				traceAst(ast);
				return TYPE_any;
			}
			if (!genAst(rt, ast->op.rhso, TYPE_any)) {
			...
			if (!emitOperator(rt, opc, got)) {	// uint % int => u32.mod
				traceAst(ast);
				return TYPE_any;
			}
			...
			switch (ast->kind) {
				...
				case OPER_leq:
				case OPER_gte:
					dieif(got != CAST_bit, ERR_INTERNAL_ERROR": got: %K / %t", got, ast);
			}
			#endif
		} break;
		...
		case OPER_any: {	// '||'
			...
			if (!emitOperator(rt, opc, CAST_u32)) {
				traceAst(ast);
				return TYPE_any;
			}
		...
		case ASGN_set: {	// '='
			ccKind cast = ast->op.lhso->type->kind & MASK_cast;
...
//			debug("%?s:%?u: static variable @%06x: %T", var->file, var->line, var->offs, var);
	...
	if (cc->root != NULL) {
		...
		if (!genAst(rt, cc->root, gStatic ? CAST_vid : TYPE_any)) {
			traceAst(cc->root);
			return 0;
		}
